...
def statistics_screen(screen):
    ...
    while True:
        ...
        if 125 <= mouse_pos[0] <= 350 and 520 <= mouse_pos[1] <= 560:  # Проверка позиции курсора
            draw_button(screen, STATISTICS_SCREEN_BUTTON, BUTTON_TARGETED_COLOR, button_font)  # и отрисовка кнопок
        else:
            draw_button(screen, STATISTICS_SCREEN_BUTTON, BUTTON_COLOR, button_font)
